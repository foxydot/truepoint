...
function msdlab_do_post_tabs() {
    ...
    while($aboutyou_metabox->have_fields('tabs')):
        ...
        $nav_tabs[$i] = '<li'.($i==0?' class="active"':'').'><a href="#'.sanitize_title(wp_strip_all_tags($aboutyou_metabox->get_the_value('title'))).'" data-toggle="tab" data-option-value=".'.sanitize_title(wp_strip_all_tags($aboutyou_metabox->get_the_value('title'))).'" onclick="_gaq.push([\'_trackEvent\',\'tab\',\'click\',\''.$aboutyou_metabox->get_the_value('title').'\',1]);"><img class="img-circle grayscale" src="'.$image[0].'" /><img class="img-circle logo-mark" src="'.get_stylesheet_directory_uri().'/lib/img/logo_mark.svg" /><h4 class="tab-title">'.$aboutyou_metabox->get_the_value('title').'</h4></a></li>';       
