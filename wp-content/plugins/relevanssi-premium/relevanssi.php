...
$relevanssi_variables['database_version'] = 16;
$relevanssi_variables['plugin_version'] = "1.11";
...
function relevanssi_negatives_positives($negative_terms, $positive_terms, $relevanssi_table) {
	...
	if ($positive_terms) {
		for ($i = 0; $i < sizeof($positive_terms); $i++) {
			$query_restrictions .= " AND doc IN (SELECT DISTINCT(doc) FROM $relevanssi_table WHERE term = '$positive_terms[$i]')";
		}
	}
